...
/* ------ Directory ----------- */
char directories[128], mkdir[256];
/* ---------------------------- */
...
{
    char filename[256];
    snprintf(filename, sizeof(filename), "%s/step_%06d.txt", directories, step); // "step_0001.txt", "step_0002.txt", ...
...
int main()
{
    ...
    snprintf(directories, sizeof(directories), "movie_data/K_%.1f_T_%.1f", K, T_res);
    snprintf(mkdir, sizeof(mkdir), "mkdir -p %s", directories);
    system(mkdir);

    // FILE *fid1, *fid2, *fid3;
    // fid1 = fopen("Energy_data_NVT_N128_rho1.20_K1.0_dt0.001a", "w");
    // fid2 = fopen("Temperature_NVT_N128_rho1.20_K1.0_dt0.001a", "w");
    // fid3 = fopen("Extended_var_NVT_N128_rho1.20_K1.0_dt0.001a", "w");
    ...
    for (int step = 0; step < max_step; step++)
    {
        ...
        if (abs(sumpx) || abs(sumpy) > 1e-10) printf("mistake detected");

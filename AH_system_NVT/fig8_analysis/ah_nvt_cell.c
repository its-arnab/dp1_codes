...
const int    max_step = (int) 1e7;                     /* No. of steps simulation run -- */
...
int main()
{
    ...
    Temperature();
    printf("Temperature of the system = %.8lf \n", T);
    printf("---------------------------------------");
    
    FILE *fid1;
    char filename1[256];
    sprintf(filename1, "Vcm_data/T_%.2f", T_res);
    ...
    for (int step = 0; step < max_step; step++)
    {
        ...
        for (int i = 0; i < N; i++) Vx_cm += - cos(theta[i]);
        for (int i = 0; i < N; i++) Vy_cm += - sin(theta[i]);
        
        Vx_cm = K * Vx_cm / (double) N;
        Vy_cm = K * Vy_cm / (double) N;

        fprintf(fid1, "%.12lf\t %.12lf\n", Vx_cm, Vy_cm);
    ...
    for (int i = 0; i < N; i++)
    {
        vx = vx + px[i] * s_inv - K * cos(theta[i]);
        vy = vy + py[i] * s_inv - K * sin(theta[i]);

        fprintf(fid2, "%.12lf\t %.12lf\n", vx, vy);
    }
    fclose(fid2);
    */
    /* -------------------------------------- */

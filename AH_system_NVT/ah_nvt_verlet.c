...
void force_calculation()
{
    ...
    for (int i = 0; i < N-1; i++)
    {
        ...
        for (int k = 0; k < tot_neigh; k++)
        {
            ...
            if (dr2 < sigma2/100.0)
            {
                printf("Is this physical distance? %.14lf \n", dr2);
                exit(1);
            }
            ...
            if (dr2 < r_c*r_c)
            {
                ...
                t0 = - J * s * int_strength * sin(theta_ij);
                ...
                /* -------------- For fx and fy -------------- */
                r_dist = pow((dr - r_c), 5);
                t0 = (int_strength * int_strength) / (dr * r_dist);
...
int main()
{
    ...
    construct_verlet_list();
    printf("Verlet List Constructed Successfully. \n");
    printf(" --------------------------------------- \n");        
    ...
    fid2 = fopen("Temperature_NVT_N128_rho1.20_K0.0_dt0.001", "w");

...
void load_final_config()
{
    ...
    fid_load = fopen("config_benchmark3.txt", "r");
    if (fid_load == NULL) {
        printf("Error: Unable to open file: config_benchmark3.txt");
        exit(1);
    }

    for ( int i = 0; i < N; i++ )
    {
        fscanf(fid_load, "%lf %lf %d",
                &x[i], &y[i], &dummy);
    }
...
int main()
{
    ...
    double force_check = 0.0;
    for (int i = 0; i < N; i++)
    {
        force_check = force_check + fx[i]*fx[i] + fy[i]*fy[i];
    }

    force_check = sqrt(force_check);
    printf("Force Disbalence: %.16lf \n \n", force_check);
    

    exit(0);   
    // I don't required anything after this point
...
! with PBC. Two importent benchmark has done :               !
! 1. Total energy is conserved upto 3 decimal points         !
! 2. Energy error flactuates as dt^2                         !
! 3. Force function checked extensively by calculate force   !
!    between two particles which match from function.        !
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
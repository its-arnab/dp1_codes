...
const double T_res = 0.10;                            /* Reserver Temperature --------- */
...
int main()
{
    ...
    double P_sum = 0.0, P_avg;
    char filename1[512], filename2[512];
    char filename3[512], filename4[512];
    ...
    /* -------- Control Directories -------------- */
    sprintf(directories, "Excess_data/N%d_rho_%.2f_K%.1f_T%.2f_dt%.3f", N, rho, K, T_res, dt);
    sprintf(mkdir, "mkdir -p %s", directories);
    system(mkdir);
    /* ------------------------------------------- */

    /* ------------------------------------------------------------------------------ */
    FILE *fid1, *fid2, *fid3;
    sprintf(filename1, "%s/Energy_data", directories);
    sprintf(filename2, "%s/Temperature_data", directories);
    sprintf(filename3, "%s/Extended_var", directories);
    ...
    for (int step = 0; step < max_step; step++)
    {
        ...
        {   
            ...
            fprintf(fid2, "%.12lf\t %.12lf\t %.12lf\t %.12lf\t %.12lf\n", 
                            T1, T2, T3, T, pressure);
    ...
    /* ------------ Saving avg -------------- */
    sprintf(filename4, "avg_data/N_%d_rho_%.2f_K_%.1f_T_%.4f_dt_%.3f", N, rho, K, T_res, dt);
    FILE *fid4;
    fid4 = fopen(filename4, "w");
    if (!fid4) printf("Error! while opening fid4");

    E_avg = E_sum / (double) max_step;
    P_avg = P_sum / (double) max_step;

    E_avg = E_avg / (double) N;     // avg. Energy per particles

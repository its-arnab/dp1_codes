...
for temp_dir in temp_dirs:
    ...
    qoft_file = os.path.join(temp_dir, f"qoft_T_{temp_str}.dat")
    if not os.path.isfile(qoft_file):
        print(f"Missing: {qoft_file}")
        continue

    # Load and plot the data
    try:
        data = np.loadtxt(qoft_file)
        times = data[:, 0]
        qoft = data[:, 1]
        ...
        print(f"Error reading {qoft_file}: {e}")
        continue

# Finalize plot
plt.xscale("log")
plt.xlabel("Time", fontsize=16)
plt.ylabel("Q(t)", fontsize=16)

...
const int    max_step = (int) 2e7;                     /* No. of steps simulation run -- */
const int    half_max_step = 0.5 * max_step;           /* half of max steps ------------ */
const double T_res = 0.10;                             /* Relax Temperature for NVT ---- */
...
int main()
{
    int swit = 0;               // switch to control , 1 (new), 0 (old)
    int nvt_flag = 0;           // NVT stop succesfully ?
    int start_Eavg = (int) (half_max_step - 1e4);
    int stop_Eavg = (int) (half_max_step - 1e3);
    double E_sum = 0.0, E_avg;
    

    ProgramInitializer(swit);
    ...
    FILE *fid1, *fid2, *fid3;
    fid1 = fopen("Energy_data_NVT_N128_rho1.20_K1.0_dt0.001", "w");
    fid2 = fopen("Temperature_NVT_N128_rho1.20_K1.0_dt0.001", "w");
    fid3 = fopen("Extended_var_NVT_N128_rho1.20_K1.0_dt0.001", "w");
    ...
    for (int step = 0; step < half_max_step; step++)
    {
        // if (step % 100 == 0)
        // save_data_infile(step);

        // debug
        double sumpy = 0.0;
        for (int i = 0; i < N; i++) sumpy += px[i];
        if (abs(sumpy) > 1e-10) printf("mistake ditected");
        ...
        if (step % 100 == 0)
        {
            fprintf(fid1, "%.12lf\t %.12lf\t %.12lf\t %.12lf\t %.12lf\n ", 
                            KE/N, RE/N, PE/N, E/N, s*(E+E_s-H_nose0)/N );
            fprintf(fid2, "%.12lf\t %.12lf\t %.12lf\t %.12lf\n", 
                            T1, T2, T3, T);
            fprintf(fid3, "%.12lf\t %.12lf\n", s, p_s);
        }

    
        if (step >= start_Eavg && step < stop_Eavg) E_sum += E;
        ...
        if (step >= stop_Eavg)
        {
            ...
            if (abs(E_avg - E) < 1e-8)
            {
                ...
                // printf("\n Avg Energy: %lf\n", E_avg/N);
                printf("\n-----------------------------------------");
                printf("\n Energy Matched. Break in step %d \n", step);
                break;
                printf("check");
            }
        }
    ...
    if (nvt_flag)
    {
        ...
        printf("------------------------------------------ \n");
        printf(" All variables changes to real co-ordinate. \n");
        printf("------------------------------------------- \n");

        force_calculation();
        update_torque();

        printf(" NVE Started Succesfully. \n");
        printf("--------------------------\n");
    }
    else
    {
        printf("\nNVT Stopped Unsuccesfully.\n");
        exit(0);
    }


    for (int step = half_max_step; step < max_step; step++)
    {
        // if (step % 100 == 0)
        // save_data_infile(step);

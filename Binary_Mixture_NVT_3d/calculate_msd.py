...
def compute_msd_for_run(run_path):
    ...
    avg_msd = np.array([msd_vals[lag_times == t].mean() for t in unique_lags])
    unique_lags *= dt

    return  unique_lags, avg_msd
...
def process_temperature(temp_path):
    ...
    for r in runs:
        ...
        result = compute_msd_for_run(run_path)
    ...
    msd_vals = np.array([res[1] for res in results])
    avg_msd = np.mean(msd_vals, axis=0)

    output_file = os.path.join(temp_path, f"msd_{os.path.basename(temp_path)}.dat")
    np.savetxt(output_file, np.column_stack((time_ref, avg_msd)), header="time Q(t)", fmt="%.8e")

/*-----------------------------------------------------*
 *        NVE simulation(PBC) with verlet list         *
 *-----------------------------------------------------*/
...
void construct_verlet_list()
{
    ...
    // for (int i = 0; i < neighbour[0].max_counter; i++)
    // {
    //     printf("%d\n", neighbour[0].list[i]);
    // }
    // printf("\n");
...
void integrate()
{
    ...
    vlist_counter ++;
    if ( (2.0 * max_disp * vlist_counter) > skin_depth )
    {
        // printf("vlist constructed after step: %d \n", vlist_counter);

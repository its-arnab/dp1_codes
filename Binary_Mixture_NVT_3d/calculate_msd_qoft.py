...
def compute_qoft_for_run(run_path):
    ...
    for block_start in range(1, step_nums[-1] + 1, blocksize):
        ...
        for n in range(1, 17):  # 2^0 to 2^16 = 65536
            ...
            msd = np.mean(squared)
            msd_data.append((lag_step - origin_step, msd))
        ...
        for target_start in block_starts[i + 1:]:
            for n in range(1, 17):  # lag steps: 2^0 to 2^16
                ...
                if target_step in step_to_index:
                    ...
                    msd = np.mean(squared)
                    msd_data.append((target_step - origin_step, msd))
    ...
    lag_times_msd = msd_data[:, 0]
    msd_vals = msd_data[:, 1]
    unique_lags_msd = np.unique(lag_times_msd)
    avg_msd = np.array([msd_vals[lag_times_msd == t].mean() for t in unique_lags_msd])
    unique_lags_msd *= dt
    ...
    q_vals = qoft_data[:, 1]
    unique_lags_qoft = np.unique(lag_times)
    avg_qoft = np.array([q_vals[lag_times == t].mean() for t in unique_lags_qoft])
    unique_lags_qoft *= dt

    return unique_lags_msd, avg_msd, unique_lags_qoft, avg_qoft
...
def process_temperature(temp_path):
    ...
    time_ref_msd = results[0][0]      # unique_lags_msd from first run
    time_ref_qoft = results[0][2]     # unique_lags_qoft from first run

    msd_vals = np.array([res[1] for res in results])   # avg_msd from each run
    qoft_vals = np.array([res[3] for res in results])  # avg_qoft from each run

    avg_msd = np.mean(msd_vals, axis=0)
    avg_qoft = np.mean(qoft_vals, axis=0)


    output_file = os.path.join(temp_path, f"msd_{os.path.basename(temp_path)}.dat")
    np.savetxt(output_file, np.column_stack((time_ref_msd, avg_msd)), header="time MSD", fmt="%.8e")
    print(f"Saved MSD: {output_file}")

    output_file = os.path.join(temp_path, f"qoft_{os.path.basename(temp_path)}.dat")
    np.savetxt(output_file, np.column_stack((time_ref_qoft, avg_qoft)), header="time Q(t)", fmt="%.8e")
